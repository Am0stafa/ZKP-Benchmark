...
impl ZKP {
    /// output = (g^secret mod p, h^secret mod p)
    pub fn generate_commitment_pair(&self, exp: &BigUint) -> (BigUint, BigUint) {
        let p1 = self.g.modpow(exp, &self.p);
        let p2 = self.h.modpow(exp, &self.p);
        (p1, p2)
    }
